<h1>Importa il ruolo {$remote.name|wash()} da {$remote_url}</h1>

{if $locale}
<div class="message-error">
    <strong>Attenzione:</strong> Il ruolo <a href="{concat('role/view/',$locale.id)|ezurl(no)}">{$locale.name} è già presente in questa installazione</a>.<br />
        ...
        {else}
            {foreach $policy.Limitation as $name => $value}
                <tr{if and( is_array($value)|not(), is_set($fixes[$value]) )} class="warning"{/if}>
                    ...
                    <td>
                        ...
                        {if is_set($fixes[$value])}
                            {if $fixes[$value].name|eq('Subtree')}
                                <p><small><strong>Valori della policy remota: </strong><br />
                                {foreach $fixes[$value].values as $item}
                                    {$item.name|wash()} ({$item.node_id|wash()}){delimiter},{/delimiter}
                                {/foreach}
                                </small></p>
                                <input type="text" value="{$fixes_data[$value]}" placeholder="Id di nodo separati da virgola" name="{$value}"/>
                            {elseif $fixes[$value].name|eq('Node')}
                                ...
                                <input type="text" value="{$fixes_data[$value]}" placeholder="Id di nodo" name="{$value}"/>
                            {elseif or($fixes[$value].name|eq('Class'), $fixes[$value].name|eq('ParentClass'))}
                                <p><small><strong>Valori della policy remota: </strong><br />
                                    {$fixes[$value].values|implode(',')}{delimiter},{/delimiter}
                                </small></p>
                                <input type="text" value="{$fixes_data[$value]}" placeholder="Id di classe separati da virgola" name="{$value}"/>
                            {/if}
                        {else}
                            {if is_array($value)}
                    ...
                    <td style="font-size: .7em">{$remote.policies[$index]['Limitation'][$name]|attribute(show,4)}</td>
                </tr>
            {/foreach}
        {/if}
    {/foreach}
